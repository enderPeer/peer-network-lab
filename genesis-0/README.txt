...
893 acts, 60 signed epoch blocks, produced between 2026-07-31 and
...
chain/blocks.jsonl and every root replays and every signature checks, with
no need to trust whoever is serving these bytes.
...
network that follows this one accepts value from exactly one source -
bitcoin destroyed at an address with no key, proven by a transaction id
anyone can check - so the two records cannot be continuous, and pretending
otherwise by carrying the old balances forward would have priced the
fiction in permanently.

The chain's block hashes were anchored into Bitcoin itself through
OpenTimestamps before this was sealed, so the dates above are not a claim
